...
namespace Components
{
	...
	CommonPatch::CommonPatch()
	{
		Events::OnDvarInit([]
		{
			...
			// < iw3xo mirror port (Stage A) - dvar registration
			Dvars::r_fullMirror = Dvars::Register::Dvar_RegisterInt(
				"r_fullMirror",
				"0: off, 1: flip world/gun BEFORE HUD (HUD intact), 2: flip everything at EndScene",
				0, 0, 2, Game::saved);
			Dvars::r_fullMirrorDepth = Dvars::Register::Dvar_RegisterInt(
				"r_fullMirrorDepth",
			...
			Dvars::r_mirrorViewmodel_rtt = Dvars::Register::Dvar_RegisterInt(
				...
				0, 0, 1, Game::saved);
			Dvars::r_mirrorViewmodel_rttBlend = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_rttBlend",
				"0:SRCALPHA/INVSRCALPHA  1:+ATE  2:ONE/ONE  3:ONE/ONE+ATE",
				2, 0, 3, Game::saved);
			Dvars::r_mirrorViewmodel_rttTonemapInject = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_rttTonemapInject",
				"Inject RTT into tonemap source (1) instead of BB composite (0)",
				1, 0, 1, Game::saved);
			Dvars::r_mirrorViewmodel_rttEarlyComposite = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_rttEarlyComposite",
				"Run RTT composite at PSCF c7 tonemap fingerprint (1=on)",
				1, 0, 1, Game::saved);
			Dvars::r_mirrorViewmodel_compositeSrgb = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_compositeSrgb",
				"sRGB encoding for composite (0:none 1:write 2:read 3:both)",
				1, 0, 3, Game::saved);
			Dvars::r_mirrorViewmodel_depthFix = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_depthFix",
				"Depth write before composite (0:off 1:both 2:far-only 3:both+near<1)",
				0, 0, 3, Game::saved);
			Dvars::r_mirrorViewmodel_clearRttDepth = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_clearRttDepth",
				"Clear RTT depth at EndScene (1=on)",
				1, 0, 1, Game::saved);
			Dvars::r_mirrorViewmodel_flipVSCF = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_flipVSCF",
				"Matrix-flip fallback when RTT=0 (0:off 1:dhp-only 2:dhp+follow)",
				0, 0, 2, Game::saved);
			Dvars::r_mirrorViewmodel_flipFollow = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_flipFollow",
				"Number of follow-up VSCF uploads to flip after dhp seen",
				0, 0, 64, Game::saved);
			Dvars::r_mirrorViewmodel_flipAxis = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_flipAxis",
				"0..3:row-flip  4..7:column-flip  8:negate-all  9:row-1-cols",
				4, 0, 9, Game::saved);
			Dvars::r_mirrorViewmodel_flipReg = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_flipReg",
				"Target VSCF register for matrix flip (0 = projection)",
				0, 0, 252, Game::saved);
			Dvars::r_mirrorViewmodel_log = Dvars::Register::Dvar_RegisterInt(
				"r_mirrorViewmodel_log",
				"Mirror logging verbosity (0:off 1:per-frame 2:verbose)",
				0, 0, 2, Game::saved);
			// iw3xo mirror port end >
		});
